...
- `OtaAgentSerialIntegrationTest`, the chain end to end with nothing injected between the pieces: a real `SerialBackend` opened on a pseudo-terminal, a real `OtaAgent` whose `SendFn` is `SerialBackend::send` and whose `frameSink()` is what the backend was built with, a real session and a real parser on both ends. `OtaAgentTest` drives a whole update through an injected lambda, so the seam that matters in production was the one seam nothing exercised. Three cases: a whole update reaching `Done` with every image byte crossing a file descriptor, a silent board reported `NoAnswer` rather than as a broken link, and the truncated `COMMIT` `ACK` firmware measured, absorbed by the resend so the update still finishes. POSIX only, the same gate `SerialBackendTest` uses.
- `FlashFrameParserTest.AnAckTruncatedByOneByteIsABadFrameNotAShortOne` (REQ-INTEGRATION-013), from a real capture rather than an invented case. A board that resets to switch banks immediately after answering leaves the last CRC byte in the shift register, so a successful `COMMIT` reaches the host as seven bytes of an eight-byte `ACK`: a succeeded update reported as a corrupt answer, then a retry of an update that already happened. The parser already handled it correctly -- held while alone, rejected as a bad frame once the next frame arrives behind it, with that frame still decoding -- and now says so under test.

#### Fixed
- `OtaSession::Timing`'s three timeout defaults were bare numbers, and 2025 in particular means nothing without the sentence beside it. They are now `kOtaAnswerTimeout`, `kOtaBeginAnswerTimeout` and `kOtaRebootWait` at namespace scope, with the resend budget named alongside them. They were written in #189 and broke nothing there, but clang-tidy reads a header through whichever translation unit includes it: the moment `OtaAgent.cpp` arrived and pulled in `OtaSession.h`, three `readability-magic-numbers` diagnostics became CI errors on a header this branch never edited.
...
TLS mode via cpp-httplib's OpenSSL support, including mutual TLS, so the one
backend with a real TLS library boundary can demonstrate both server-only and
client-verified deployments without an external stunnel process. ADR-0030.
...
- `EquipmentStateReading`: maps a `equipment/<n>/state` reading with an on/off value onto `ProductionModel::setEquipmentEnabled`, mirroring the MQTT `SensorIngestBridge` vocabulary. Unit-tested in isolation.
- Wired the serial backend into `buildIntegrationServices` behind `network.serial.enabled` (default false) and `BUILD_SERIAL_BACKEND` (default OFF), with the port device and baud read from config.
- `docs/uml/sequence-serial-read.puml`: the MCU-to-GTK serial ingest flow, with `SerialBackend` added to the integration class and component-overview diagrams.
...
- `GET /production`: throughput + OEE + derived `minutesPerUnit`, served through a shared `ProductionMetricsJson` builder the MCP `production_metrics` tool now delegates to, so the REST route and the LLM tool cannot drift (REQ-INTEGRATION-008).
- `/alarms` activated in the GTK and console binaries: `buildIntegrationServices` now receives the process-wide `AlertCenter` (the same store MainWindow / InitConsole raise alarms into), so the route serves live alarms when `network.http.enabled` is set. The Qt frontend already passed its AlertCenter.

#### Changed
- `TcpBackend`'s `status` command now calls the shared `buildStatusJson` instead of building the JSON inline.
- `ProductionMetricsTool` now delegates to the shared `buildProductionMetricsJson`, preserving its public API and behaviour.
- `MainWindow` borrows the `AlertCenter` from `Application::getAlertCenter()` when the composition root supplies one, else owns its own (standalone GUI tests unchanged); `InitConsole` takes the `AlertCenter` by reference from `main()`.
