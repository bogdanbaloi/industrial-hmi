...
- Wired the serial backend into `buildIntegrationServices` behind `integration.serial.enabled` (default false) and `BUILD_SERIAL_BACKEND` (default OFF), with the port device and baud read from config.
