...
- **Real OEE composite** (Availability * Performance * Quality, Vorne
  ...
  REQ-DASHBOARD-008.
- **Requirements traceability (OpenFastTrace)** -- <!--fig:adrs-->34<!--/fig--> ADRs + <!--fig:reqs-->93<!--/fig-->
...
- **67% test coverage** verified by gcovr in CI on every PR, across
  11,112 instrumented lines and **<!--fig:ctest-->102<!--/fig--> ctest targets**: scenario-based
...
**Size.** `src/` holds **<!--fig:loc-->29,378<!--/fig--> lines of C++ code across <!--fig:files-->321<!--/fig--> files**, counted
...
**Test count.** The **<!--fig:ctest-->102<!--/fig--> ctest targets** quoted
...
| Plain TCP, no TLS (MQTT)  | MQTT only: production deployments tunnel it through stunnel, wire format unchanged. Not a repo-wide caveat. The HTTP/REST backend terminates TLS itself, mutual TLS included (`src/integration/README.md`, ADR-0030), and the OPC-UA endpoints speak Basic256Sha256 Sign&Encrypt (ADR-0031) |
...
| Testing | GoogleTest + gmock (<!--fig:ctest-->102<!--/fig--> ctest targets on a default configure) + google/benchmark (p50/p90/p99 hot-path microbenchmarks) + libFuzzer (wire-parser fuzz harnesses) |
