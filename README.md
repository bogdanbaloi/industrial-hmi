...
- **Real OEE composite** (Availability * Performance * Quality, Vorne
  ...
  REQ-DASHBOARD-008.
- **Requirements traceability (OpenFastTrace)** -- 29 ADRs + 87
...
- **67% test coverage** verified by gcovr in CI on every PR, across
  11,112 instrumented lines and **98 ctest targets**: scenario-based
...
tests/                  98 ctest targets (see Testing section)
...
**Size.** `src/` holds **27,984 lines of C++ code across 308 files**, counted
...
| Testing | GoogleTest + gmock (98 ctest targets) + google/benchmark (p50/p90/p99 hot-path microbenchmarks) + libFuzzer (wire-parser fuzz harnesses) |
