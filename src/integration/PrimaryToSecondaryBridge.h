...
/// "primary" ProductionModel onto a "secondary" ProductionModel.
///
...
///
/// Cross-process equivalent (primary and secondary on different machines)
...
///
/// SOLID:
///   * S -- one job: forward primary equipment-status events into
///     secondary's setEquipmentSupplyLevel setter. No threading of its own,
///     no protocol, no UI.
...
///
/// Threading: primary's onEquipmentStatusChanged callback fires on the
...
/// The secondary's setEquipmentSupplyLevel is the same entry point every
/// other ingest bridge uses; ProductionModel guarantees thread safety
/// for setters.
///
/// Lifecycle:
///   * start() subscribes to primary and flips isRunning_ true.
...
public:
    ...
    /// the secondary since construction. Used in tests and in the metrics
    /// tooltip.
    [[nodiscard]] std::size_t forwardedCount() const noexcept;

private:
    /// Callback installed on primary at start(). Forwards the supply
    /// level into secondary's setEquipmentSupplyLevel setter when the
