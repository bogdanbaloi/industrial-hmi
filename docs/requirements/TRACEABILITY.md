...
| REQ-ARCH-018 | SHOULD | `src/mcp/McpProtocol.h/.cpp`, `src/mcp/McpError.h`, `src/mcp/tools/AlarmsSnapshotTool.h/.cpp`, `src/mcp/tools/HistorianQueryTool.h/.cpp`, `src/mcp/McpServer.h/.cpp`, `src/mcp/McpInitRoot.h/.cpp`, `src/mcp/main.cpp`, `CMakeLists.txt` | McpProtocolTest.* (initialize / tools-list / both read-only tools over AlertCenter + a stub HistoryReader / argument validation / boundary error mapping); manual run of `industrial-hmi-mcp` against an MCP client | 0001, 0003, 0014, 0020, 0023 |
